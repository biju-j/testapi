...
*
* supportapi.createjsblipp(bRole, projid, Name, callback)
*
*
**/
var createjsblipp = function(role, projid, name, callback) {
            var userrole=role||'blippar_admin';
            var token = users.ROLES[userrole].apiTOKEN;
            var mdata = { "name": name, "type": "customjs"   }
            var options = {
                method: "POST",
                url: "https://"+baseurl+"/api/campaign/"+projid+"/js/blipp",
           ...
           //logger.info("URL of CustomJS "+options.url);
...
/** Generic reusable method.
*
*   For New Version Creation bRole, blippid and blipptype to be bespoke, builder_v2
*   Eg: supportapi.createversion(bRole, blippid, 'bespoke', callback)
*
**/
var createversion = function(role, blippid, type, amarker, callback) {
            var userrole=role||'blippar_admin';
            var token = users.ROLES[userrole].apiTOKEN;
            if(type == 'builder_v2'){
                var versionapi = '/api/blipp/'+blippid+'/version?limit=0&type='+type;
                ...
                try{
                      ...
                      console.log('VERSION Creation Error');
                      callback(e);
                }
                    ...
                    var options = {
                        method: "POST",
                        url: "https://"+baseurl+"/api/blipp/"+blippid+"/version?limit=0&type="+type,
                        headers: { "content-type":"multipart/form-data",
                                   "authorization": token,
                                   "cache-control": "no-cache"
                                  },
            ...
            else if(type == 'bespoke_country'){
                   var mdata = { file: fs.createReadStream(__dirname + amarker), country: 'in'}
                   var options = {
                            method: "POST",
                            url: "https://"+baseurl+"/api/blipp/"+blippid+"/version?limit=0&type="+'bespoke',
                            headers: { "content-type":"multipart/form-data",
                                       "authorization": token,
                                       "cache-control": "no-cache"
                                     },
            ...
            else if(type ==  'blipp_api1'){
                   var versionapi = '/api/blipp/'+blippid+'/js/version';
                   var jsidata = { "reusemarkers": "true", "type": "customjs"}
                   var options = {
                                   method: "POST",
                                   url: "https://"+baseurl+"/api/blipp/"+blippid+"/js/version",
                                   headers: { "content-type":"multipart/form-data",
                                              "authorization": token,
                                              "cache-control": "no-cache"
                                             },
                   ...
                   request(options, function(err, response, body) {
                         //logger.info("Blipp_api initial Version "+response.statusCode);
                      if (err)
                       callback(err);
                      callback(body);
                   });
            ...
            else if(type ==  'blipp_api2'){
                  var filepath = fs.createReadStream(__dirname + amarker);
                  var jdata = { "file": filepath ,"reusemarkers" : "true", "publishmode":"live"}
                  var options = {
                                  method: "POST",
                                  url: "https://"+baseurl+"/api/blipp/"+blippid+"/js/flow/customjsversion",
                                  headers: { "content-type":"multipart/form-data",
                                              "authorization": token,
                                              "cache-control": "no-cache"
                                           },
                  ...
                  request(options, function(err, response, body) {
                         //logger.info("Blipp_api2 Version "+response.statusCode);
                     if (err)
                        callback(err);
                     callback(body);
                  });
...
/** Publish Blipp API blipp
*
*  jsapiid, versionid, country to be published Eg: 'IN'
*
**/
var publishjsblipp = function(role, blippid, versionid, country, callback) {
           var userrole=role||'blippar_admin';
           var token = users.ROLES[userrole].apiTOKEN;
           var mdata = { "code": versionid, "country": country, "publishmode": "live", "version": ''   }
           var options = {
                            method: "POST",
                            url: "https://"+baseurl+"/api/blipp/"+blippid+"/js/publish",
                            headers: { "content-type":"multipart/form-data",
                                       "authorization": token,
                                       "cache-control": "no-cache"
                                     },
...
/**
*
* Empty plist creation blippid, blippversionid created should be passed
*
***/

var createplist = function(role, blippid, blippversionid, callback){
             var userrole=role||'blippar_admin';
             var token = users.ROLES[userrole].apiTOKEN;
             var emptyplist ='/../resources/assets/empty_plist';
             var mdata = { file: fs.createReadStream(__dirname + emptyplist) }
             var plistapi = '/api/blipp/'+blippid+'/version/'+blippversionid+'/plist';
             var options = {
                             method: "POST",
                             url: "https://"+baseurl+plistapi,
                             headers: { "authorization": token, "cache-control": "no-cache"  },
                             formData : mdata
                           };
             ...
             request(options, function(err, response, body) {
                     if (err)
                        callback(err);
                     // console.log("plist response> "+response.statusCode);
                     callback(body);
             });
...
SupportAPI.prototype.createjsblipp = createjsblipp;
SupportAPI.prototype.createversion = createversion;
SupportAPI.prototype.createmarker = createmarker;
SupportAPI.prototype.createplist = createplist;
SupportAPI.prototype.publishjsblipp = publishjsblipp;
